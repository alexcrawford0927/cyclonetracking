...
###########################'''
def detectMinima(var,kSize=3,nanthreshold=1):
    ...
    '''
    # Find percentage of Nans in each neighbor
    nancount = ndimage.uniform_filter(np.isnan(var).astype(np.float),kSize)

    # Find the local minima
    output = ndimage.minimum_filter(var,kSize)
    
    # export valid locations as 1s and invalid locations as 0s
    return (output == var) & (nancount < nanthreshold)

def detectMaxima(var,kSize=3,nanthreshold=1):
...
###########################'''
def findCenters(field, kSize=3, nanthreshold=0.5, d_slp=0, d_dist=0, yDist=0, xDist=0):
    ...
    fieldMinima = detectMinima(field,kSize,nanthreshold=nanthreshold).astype(np.uint8)
