...
regpath = path+"/Projections/EASE2_N0_100km_GenesisRegions.nc"
...
mindisp = 0 # the maximum displacement from the genesis point must be at least this large (in km)
...
# Create Empty lists
sid, tid, year, month, avguv, maxdsqp, minp, maxdepth, avgdsqp, avgp, avgdepth = [[] for i in range(11)]
...
while mt != endtime:
    ...
    for tr in trs: # For each track
        ...
        maxdepth.append( tr.data.depth.max() ), avgdsqp.append( tr.data.DsqP.mean() )
        ...
        avgarea.append( tr.avgArea() ), mcc.append( tr.mcc() )
        spl1.append( spl1_count ), spl2.append( spl2_count ), spl3.append( spl3_count )
        mrg1.append( mrg1_count ), mrg2.append( mrg2_count ), mrg3.append( mrg3_count ), rge2.append( rge2_count )
...
if kind1 == 'System':
    ...
    "maxdsqp":maxdsqp,"minp":minp,"maxdepth":maxdepth,"avgdsqp":avgdsqp,\
    "avgp":avgp,"avgdepth":avgdepth,"lifespan":lifespan,"trlen":trlen,\
    "avgarea":avgarea,"mcc":mcc,"spl1":spl1,"spl2":spl2,"spl3":spl3,\
    "mrg1":mrg1,"mrg2":mrg2,"mrg3":mrg3,"rge2":rge2,"genReg":genReg,"lysReg":lysReg})
else:
