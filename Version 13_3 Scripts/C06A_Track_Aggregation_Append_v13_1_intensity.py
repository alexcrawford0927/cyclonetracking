...
Date Modified: 22 Feb 2021: Adpated from prior C4 scripts to work with just intensity measures
            ...
            10 Jan 2024: Changed from square to circle smoothing kernel -- takes longer, but looks prettier
            23 Jan 2025: Fixed a bug so that now the code can handle each input varibale having a different 
            amount of pre-exisitng years already run.
...
kSizekm = 800 # Full kernel size (in km) for spatial averaging measured between grid cell centers.
    ...
    ## and the smoothing is done on a 200 km radius
...
vNames = ["countP"] + ["p_cent","depth","DsqP","p_grad","radius"]
varsi = [0] + [1,2,3,4,5]
multiplier = [1] + [0.01,0.01,100/spres/spres,1000*1000/100,spres]
# Note, for Laplacian, this is really *100*100/100/spres/spres b/c the units are Pa/gridcell --> hPa/[100 km]^2
vunits = ['percent'] + ['hPa','hPa','hPa/[100 km]^2','hPa/[1000 km]','km']
...
firstyears, nextyears = [starttime[0] for i in vNames], [endtime[0] for i in vNames] # Set to the same by default
for v in varsi:
...
# Create a 2-D kernel
kernel = md.circleKernel(int(kSizekm/spres),masked_value=0)
...
while mt != newendtime:
    ...
    # Append to main list
    field0sm = np.array( ndimage.generic_filter( fields[0], np.nansum, footprint=kernel, mode='nearest' ) )
    vlists[0].append( field0sm/n*100 ) # convert count to a %
    for vi in varsi[1:]:
        fieldsm = np.array( ndimage.generic_filter( fields[vi], np.nansum, footprint=kernel, mode='nearest' ) ) / field0sm
