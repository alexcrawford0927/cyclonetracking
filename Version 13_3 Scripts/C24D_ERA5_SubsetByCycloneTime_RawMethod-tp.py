...
for vi in [1]:
    ...
    for i in range(len(ncvars[vi])):
        ...
        if cycver+"_AggregationFields_Monthly_"+ncvars[vi][i]+".nc" in md.listdir(outpath):
            ...
            outds[ncvars[vi][i]+"-cyc"] = (['time','y','x'], outarr)
            outds[ncvars[vi][i]+"-cyc"].attrs = {'long_name':longnames[vi][i]+ " within " + str(rad) + " km of a cyclone center", 'units':units[vi][i]}
    
            # Total precip
            outarr = outds[ncvars[vi][i]].data
            outarr[np.in1d(outds['time'], monthlytimelist),:,:] = monthlybaselists[i]
            outds[ncvars[vi][i]+"-all"] = (['time','y','x'], outarr)
            outds[ncvars[vi][i]+"-all"].attrs = {'long_name':longnames[vi][i]+ " within " + str(rad) + " km of a cyclone center", 'units':units[vi][i]}
        ...
        ## If this is a new variable ##
            ...
            # Cyc precip
            outds[ncvars[vi][i]+"-cyc"] = (['time','y','x'], np.array(monthlylists[i]))
            outds[ncvars[vi][i]+"-cyc"].attrs = {'long_name':longnames[vi][i]+ " within " + str(rad) + " km of a cyclone center", 'units':units[vi][i]}
            
            # Total precip
            outds[ncvars[vi][i]+"-all"] = (['time','y','x'], np.array(monthlybaselists[i]))
            outds[ncvars[vi][i]+"-all"].attrs = {'long_name':longnames[vi][i], 'units':units[vi][i]}        
