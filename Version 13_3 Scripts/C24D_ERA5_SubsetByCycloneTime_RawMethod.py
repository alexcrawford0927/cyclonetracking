...
vi = 5
V = 'V7'
rad = 1000 # radius for constant kernel size (units: km)
...
aggtype = [['mean'],['sum'],['sum'],['sum'],['mean'],['mean','mean']]
...
while mt != endtime:
    ...
    for ct in cts:
        ...
        if len(hours) > 0:
            ...
            for h in range(len(hours))[:]:
                ...
                # Add 1 to the cyclone count
                narr[t,(y-ky):(y+ky+1),(x-kx):(x+kx+1)] = kernel 
    ...
    for i in range(len(ncvars[vi])):
        ...
        if aggtype[vi][i] == 'sum':
            ...
            monthlylists[i].append( (outarrs[i]*narr).sum(axis=0) / narr.sum(axis=0) )
    
    # Include count variable and time variable
    monthlylists[-1].append( narr.mean(axis=0) * len(times) * 24 * (times[-1] - times[-2]) )
...
# Count Variable
# Create coordinates
# if cycver+"_AggregationFields_Monthly_cychours.nc" in md.listdir(outpath):
#     outds = xr.open_dataset(outpath+"/"+cycver+"_AggregationFields_Monthly_cychours.nc")

#     outarr = outds['cychours'].data
#     outarr[np.in1d(outds['time'], monthlytimelist),:,:] = monthlylists[-1]
#     outds['cychours'] = (['time','y','x'], outarr)
#     outds['cychours'].attrs = {'long_name':'Cyclone Hours', 'units':'total hours'}
    
#     outds.to_netcdf(outpath+"/"+cycver+"_AggregationFields_Monthly_cychours_NEW.nc")
#     outds.close()
#     os.remove(outpath+"/"+cycver+"_AggregationFields_Monthly_cychours.nc")
#     os.rename(outpath+"/"+cycver+"_AggregationFields_Monthly_cychours_NEW.nc",outpath+"/"+cycver+"_AggregationFields_Monthly_cychours.nc")
...
#     # Add variables
#     outds['cychours'] = (['time','y','x'], np.array(monthlylists[-1]))
#     outds['cychours'].attrs = {'long_name':'Cyclone Hours', 'units':'total hours'}
#     outds.to_netcdf(outpath+"/"+cycver+"_AggregationFields_Monthly_cychours.nc")
#     outds.close()
...
for i in range(len(ncvars[vi])):
    ...
    ## If this is a new variable ##
        ...
        outds[ncvars[vi][i]] = (['time','y','x'], np.array(monthlylists[i]))
        outds[ncvars[vi][i]].attrs = {'long_name':longnames[vi][i], 'units':units[vi][i]}
